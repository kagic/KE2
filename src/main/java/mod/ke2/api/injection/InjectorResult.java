...
public class InjectorResult {
	...
	public static InjectorResult create(World world, BlockPos pos, boolean generate, UUID owner, int color) {
		...
		double random = Math.random() * volume * InjectorResult.CRUX_MARGIN;
		...
		ExitHole exit = null;
		float angle = world.rand.nextFloat();
		if (generate) {
	...
	public static void drainSlab(World world, BlockPos pos, Block[] blocks) {
		if (blocks == null) {
			...
			if (world.provider.isNether()) {
				blocks = InjectorResult.RED_DRAIN_SLABS;
			} else {
				float temp = world.getBiome(pos).getTemperature(pos);
				if (world.canSnowAt(pos, false)) {
					blocks = InjectorResult.BLUE_DRAIN_SLABS;
				} else if (temp > 0.95) {
					blocks = InjectorResult.ORANGE_DRAIN_SLABS;
				} else {
					blocks = InjectorResult.PURPLE_DRAIN_SLABS;
				}
			}
